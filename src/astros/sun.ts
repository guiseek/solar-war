...
export class Sun {
  ...
  loader;
  animate;
  // @ts-expect-error -- TODO: Member 'corona' implicitly has an 'any' type.
  corona;
  // @ts-expect-error -- TODO: Member 'sunRim' implicitly has an 'any' type.
  sunRim;
  // @ts-expect-error -- TODO: Member 'glow' implicitly has an 'any' type.
  glow;
  sunTexture: string;
  ...
  createSun() {
    ...
    const sunMaterial = new MeshToonMaterial({
      map,
      emissive: new Color(0xff9900),
      emissiveIntensity: 2,
    });
  ...
  createCorona() {
    ...
    let v3 = new Vector3();
    let p = new Vector3();
    let pos = coronaGeometry.attributes.position as BufferAttribute;
  ...
  createGlow() {
    const uniforms = {
      ...
      color2: { value: new Color(0xff9100) },
  ...
  createRim() {
    const uniforms = {
      color1: { value: new Color(0xff9900) },
  ...
  addLighting() {
    const sunLight = new PointLight(0xff9900, 100);
